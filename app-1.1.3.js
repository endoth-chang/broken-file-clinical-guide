...
const APP_CONFIG=window.BROKEN_FILE_TRAINING_CONFIG||{};
const PASSWORD_HASH=(APP_CONFIG.passwordHash||"f376e8c61d025645c37933b5cad900022f76df122a6e26455005e81cb8d8cbf9").trim().toLowerCase();
const ACCESS_PASSWORD=(APP_CONFIG.accessPassword||"endosi").trim();
...
function renderCase(id,phaseIndex=null){
  ...
  restoreChecks(c.id);
  window.scrollTo(0,0);
  if(phaseIndex!==null) requestAnimationFrame(()=>{const target=document.querySelector(`#phase-${phaseIndex}`);if(target)target.scrollIntoView({behavior:"smooth"});});
}
